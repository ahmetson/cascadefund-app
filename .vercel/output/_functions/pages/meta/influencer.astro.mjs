...
import { $ as $$LandingLayout, a as $$HeroSection, b as $$BaseSection } from '../../chunks/HeroSection_ZpuMqOVZ.mjs';
import { $ as $$LeftImageRightPanelsSection } from '../../chunks/LeftImageRightPanelsSection_C0Y7HK1y.mjs';
import { B as BgType, g as getIcon } from '../../chunks/eventTypes_PbqAZmEg.mjs';
import { $ as $$HorizontalPanelsSection } from '../../chunks/HorizontalPanelsSection_CRPO7Pqh.mjs';
import { B as BasePanel } from '../../chunks/Panel_CqTuM3BW.mjs';
export { renderers } from '../../renderers.mjs';

const $$Influencer = createComponent(($$result, $$props, $$slots) => {
  const heroSection = {
    title: "Help Open Source Thrive \u2014 become an influencer, and connect with people",
    children: "Your support goes beyond donations. With Ara, you can help maintainers, influence priorities, and network with the projects you may want to work with.<br />Ara has maintainer's workflow features only. Subscribe when the app profile and project pages are ready. You can still browse projects, see what maintainers want to be spread out, and tell us your favorite project maintainers to join once launched. If they joined, you can see their public posts in Discord.",
    actions: [
  ...
  const projectStats = [
    {
      ...
      description: "Since the Ara launch, there are 12 signups from people who want to be notified."
    },
    {
      number: "3",
      label: "projects",
      period: "In the last week",
      description: "There are three projects added on Ara since it launch, check them out."
    },
    {
      number: "28,465",
      label: "votes",
  ...
  return renderTemplate`${renderComponent($$result, "V2Layout", $$LandingLayout, {}, { "default": ($$result2) => renderTemplate` ${renderComponent($$result2, "HeroSection", $$HeroSection, { ...heroSection })} ${renderComponent($$result2, "LeftImageRightPanels", $$LeftImageRightPanelsSection, { "cols": 3, "prefix": "influencerChallanges", "title": "Challenges Users Face", "img": {
    src: "https://dummyimage.com/400x400/f5f5f5/000000?text=Statue+of+Liberty+with+Chains",
    alt: "Challenges illustration"
  }, "panels": challenges }, { "default": ($$result3) => renderTemplate` ${maybeRenderHead()}<p class="text-xl text-gray-600">
Open source is powerful, but being a contributor isn't always easy.
</p> ` })} ${renderComponent($$result2, "Section", $$BaseSection, { "title": solutionSection.title, "bg": solutionSection.bg }, { "default": ($$result3) => renderTemplate` <p class="text-lg text-gray-200 mb-8">
...
</p> ` })} ${renderComponent($$result2, "HorizontalPanels", $$HorizontalPanelsSection, { "cols": 2, "bottomPanel": bottomStep, "panels": steps, "bg": { sectionBgType: BgType.infoSection }, "title": "How Ara Works?" })} ${renderComponent($$result2, "Section", $$BaseSection, { "title": uniqueFeature.title, "bg": uniqueFeature.bg, "icon": getIcon("energy") }, { "default": ($$result3) => renderTemplate` <p class="text-lg text-gray-200 mb-8">
Ara's open-source, transparent management system makes it easy to track
      progress. And if a maintainer doesn't want to continue on a project, your
      voting power will even help make a fork that builds your vision.
</p> ` })} ${renderComponent($$result2, "HorizontalPanels", $$HorizontalPanelsSection, { "cols": 3, "panels": benefits, "bg": { sectionBgType: BgType.infoSection }, "title": "Why Talk to Devs Through Ara?" })} ${renderComponent($$result2, "Section", $$BaseSection, { "actions": [
    {
      variant: "primary",
      children: "Sign In as Influencer",
      uri: "/login?role=influencer"
    },
    {
      variant: "secondary",
      children: "Browse Projects",
      uri: "/data/projects?role=influencer"
    }
  ], "icon": getIcon("dev"), "title": "Start using Ara", "bg": { sectionBgType: BgType.normalSection, overlay: "rectangles" } }, { "default": ($$result3) => renderTemplate`  <div class="flex items-start space-x-4 text-left max-w-2xl mx-auto"> <div class="w-8 h-8 bg-yellow-400 rounded-full flex items-center justify-center flex-shrink-0 mt-1"> ${getIcon("person")} </div> <div> <h3 class="font-semibold mb-1">
Follow Ara project to see what feature is added
...
By getting upvote votes, you could request your own features. Upon
          ...
          points. But, first you need to be a registered user of Ara. And then
          obtain voting power for each project!
</p> </div> </div> ` })} ${renderComponent($$result2, "HorizontalSection", $$HorizontalPanelsSection, { "cols": projectStats.length, "title": "", "className": "py-16 bg-blue-gradient" }, { "default": ($$result3) => renderTemplate` <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8"> <div class="grid grid-cols-1 md:grid-cols-3 gap-8"> ${projectStats.map((stat, index) => renderTemplate`${renderComponent($$result3, "BasePanel", BasePanel, { "key": index.toString(), "className": "text-center" }, { "default": ($$result4) => renderTemplate` <div class="mb-4 round-lg border-round-lg bg-green-50"> <div class="text-3xl font-bold text-cascade-blue mb-1"> ${stat.number} </div> <div class="text-lg text-gray-700">${stat.label}</div> <div class="text-sm font-medium text-gray-900 mb-2"> ${stat.period} </div> </div> <p class="text-sm text-gray-600 leading-relaxed"> ${stat.description} </p> ` })}`)} </div> </div> ` })} ` })}`;
}, "/home/medet/ara-app/src/pages/meta/influencer.astro", void 0);

const $$file = "/home/medet/ara-app/src/pages/meta/influencer.astro";
